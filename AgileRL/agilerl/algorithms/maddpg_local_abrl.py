...
class MADDPG:
    ...
    def __init__(
        ...
        assert isinstance(reg_params, dict), "Regularization parameters must be a list."
        assert isinstance(
            one_hot, bool
        ...
        assert isinstance(lambda_, float), "Lambda_ must be a float."
        ...
        self.affinity_indices = affinity_indices
        self.affinity_condition = affinity_condition
        self.device = device
        ...
        self.scores = []
        self.fitness = []
        self.L = {agent:[] for agent in agent_ids}
        self.steps = [0]
    ...
    def _learn_individual(
        ...
        if self.arch == "mlp":
            ...
            affinity_loss = 0
            pi_i = actor(states[agent_id])
            #print('actor_loss: {}'.format(actor_loss))
            for pi, state in zip(pi_i, states[agent_id]):
                for affinity_index in self.affinity_indices[agent_id]:
                    act, cond = self.affinity_condition(state, affinity_index)
                    #print("action checked", act, "present in options?", cond)
                    if cond:
                        #print("condition satisfied")
                        #P_a = torch.tensor(self.reg_params[agent_id], dtype=torch.float).to(self.device)
                        tot_acts = len(self.reg_params[agent_id])
                        #print("total actions", tot_acts)
                        P_a = np.zeros(tot_acts, dtype=float)
                        P_a[act-1] = max(self.reg_params[agent_id])

                        # Distribute the remaining probability equally among the other indices
                        remaining_probability = (1 - P_a[act-1]) / (tot_acts - 1)
                        for i in range(tot_acts):
                            if i != act-1:
                                P_a[i] = remaining_probability
                        P_a = torch.tensor(P_a, dtype=torch.float).to(self.device)
                        #print(pi, state)
                        #print(P_a)
                        affinity_loss += self.lambda_ * self.criterion(P_a, pi)
            
            if affinity_loss != 0:
                #print(actor_loss, affinity_loss, actor_loss + affinity_loss)
                actor_loss += affinity_loss
                self.L[agent_id].append(affinity_loss.cpu().detach().numpy())
            #print('affinity_loss: {}'.format(affinity_loss))
