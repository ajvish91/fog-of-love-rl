...
class Run:
    def __init__(self, check_affinity_condition, gpu, aff_indices_1, aff_indices_2, is_abrl=False, debug=False):
        ...
        self.memory = MultiAgentReplayBuffer(memory_size=1_000_000,
        ...
        NET_CONFIG = {
            ...
            'hidden_size': [1024, 1024, 1024]  # Network hidden size
        }
        ...
        if is_abrl:
            ...
            self.affinity_indices = {"player_1": aff_indices_1, "player_2": aff_indices_2}
            self.affinity_condition = check_affinity_condition
            self.agent = MADDPG(state_dims=self.state_dim,
                            ...
                            reg_params=self.reg_params, lambda_=self.lambda_,
                            device=self.device,
        ...
        if not debug:
            if is_abrl:
                self.wandy = wandb.init(project=self.project, config={"model_name": "maddpg",
        ...
        self.avg_scores = []
        self.losses = []
        self.max_steps = 100 # For atari environments it is recommended to use a value of 500
        self.epsilon = 1.0
        self.eps_end = 0.1
        ...
        if debug:
            self.test_num_episodes = 40
        else:
            self.test_num_episodes = 200
        self.test_scores = []
...
def check_affinity_condition(state, affinity_idx):
    """
    Evaluates the affinity condition based on the goal and options in the state array.

    Parameters:
    state (list): The state array containing various values.
    affinity_idx (int): The index in the state array representing the goal.

    Returns:
    tuple: A tuple containing the option index (1, 2, or 3) and a boolean indicating if the condition is met.
    """
    goal = state[affinity_idx]
    # Extract options using slicing for memory efficiency
    ...
    # Check if the goal meets the criteria
    if (goal_positive and goal > state_value) or (not goal_positive and goal < state_value):
        #print(f"goal {'positive' if goal_positive else 'negative'}", goal)
        # Iterate through options to find a matching condition
        for i, option in enumerate(options, start=1):
            if (goal_positive and option > 0) or (not goal_positive and option < 0):
                #print(f"option {i} {'positive' if goal_positive else 'negative'}", option)
                return i, True
        #print("options do not contain goal")
        return 0, False

    #print("goal does not meet criteria")
    return 0, False
...
def validate_zero_or_one(value):
    ...
    if ivalue not in [0, 1]:
        raise argparse.ArgumentTypeError(f"{value} is not 0 or 1")
    return ivalue

def validate_zero_to_twelve(value):
    ivalue = int(value)
    if ivalue < 0 or ivalue > 12:
        raise argparse.ArgumentTypeError(f"{value} is not between 0 and 12")
    return ivalue


# Function to split the list based on a delimiter
def split_lists(numbers, delimiter):
    try:
        delimiter_index = numbers.index(delimiter)
        list1 = numbers[:delimiter_index]
        list2 = numbers[delimiter_index + 1:]
        return list1, list2
    except ValueError:
...
def validate_list_elements(lst, min_val, max_val):
    for item in lst:
...
if __name__ == "__main__":
    # Create the parser
    parser = argparse.ArgumentParser(description="Process two lists of integers.")

    # Add the arguments
    parser.add_argument('is_debug', nargs=1, type=validate_zero_or_one, help='Enter whether to debug value 0 or 1')
    parser.add_argument('is_abrl', nargs=1, type=validate_zero_or_one, help='Enter whether to use affinities value 0 or 1')
    parser.add_argument('gpu', nargs=1, type=validate_zero_to_twelve, help='Enter gpu number (0-12)')
    parser.add_argument('numbers', nargs='+', help='A list of integers separated by a delimiter')

    # Parse the arguments
    args = parser.parse_args()

    # Extract the initial numbers
    is_debug = bool(args.is_debug[0])
    is_abrl = bool(args.is_abrl[0])
    gpu = str(args.gpu[0])

    # Join the remaining arguments into a single string
    numbers_str = ' '.join(args.numbers)


    # Split the input string into a list
    numbers = numbers_str.split()

    # Convert the input strings to integers where possible
    numbers = [int(x) if x.isdigit() else x for x in numbers]

    # Use '_' as the delimiter
    delimiter = '_'
    try:
        aff_indices_1, aff_indices_2 = split_lists(numbers, delimiter)
        
        
        # Validate list elements
        validate_list_elements(aff_indices_1, 14, 20)
        validate_list_elements(aff_indices_2, 14, 20)
        
        print("Initial Numbers:", is_debug, is_abrl, gpu)
        print("List 1:", aff_indices_1)
        print("List 2:", aff_indices_2)
    except ValueError as e:
        print(e)

    print(gpu, aff_indices_1, aff_indices_2, is_abrl, is_debug)
    run = Run(check_affinity_condition, gpu, aff_indices_1, aff_indices_2, is_abrl, is_debug)
    run.train()
    run.test()
    run.plot_analysis()
    ...
    if is_abrl:
        ...
        #for (success_key, success_value), (failure_key, failure_value) in zip(affinity_success, affinity_failure):
        #    if success_value + failure_value != 0:
        #        print(success_key + "success rate", success_value/(success_value + failure_value))
