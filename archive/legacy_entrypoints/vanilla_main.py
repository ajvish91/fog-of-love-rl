...
class Run:
    def __init__(self, check_affinity_condition, gpu, aff_idx, is_abrl=False, debug=False):
        ...
        if is_abrl:
            ...
            inputs1 = aff_idx[0]
            inputs2 = aff_idx[1]
            player_1_params = [inputs1[1] if x == inputs1[0] else (1 - inputs1[1]) / (len(inputs1)) for x in range(3)]
            player_2_params = [inputs2[1] if x == inputs2[0] else (1 - inputs2[1]) / (len(inputs2)) for x in range(3)]
...
def _run_vanilla_main(debug, abrl, gpu, aff_idx_1, aff_idx_2):
    print(abrl, debug, gpu, aff_idx_1, aff_idx_2)
    run = Run(check_affinity_condition, gpu, [aff_idx_1, aff_idx_2], abrl, debug)
    run.train()
    run.test()
    run.plot_analysis()
...
if __name__ == "__main__":
    if len(sys.argv) >= 3 and sys.argv[1] in ("--config", "-c"):
        ...
        try:
            from fol.train_config import load_yaml_mapping, parse_vanilla_global

            raw = load_yaml_mapping(cfg_path)
            debug, abrl, gpu, aff_idx_1, aff_idx_2 = parse_vanilla_global(raw, cfg_path)
        except Exception as e:
            print(f"Config error: {e}", file=sys.stderr)
            sys.exit(1)
        _run_vanilla_main(debug, abrl, gpu, aff_idx_1, aff_idx_2)
        sys.exit(0)

    print(sys.argv)
    if len(sys.argv) != 6:
        print(
            "Usage: python vanilla_main.py <debug==0|1> <abrl==0|1> <gpu==0-11> "
            "<aff_idx_1==0-2> <aff_idx_2==0-2>\n"
            "   or: python vanilla_main.py --config configs/smoke_vanilla_global.yaml"
        )
        sys.exit(1)
    debug = sys.argv[1]
    ...
    if int(aff_idx_1) < 0 or int(aff_idx_1) > 2:
        print("Invalid input for aff_idx_1" + aff_idx_1)
        sys.exit(1)
    else:
        aff_idx_1 = [int(aff_idx_1), 1 / 3]

    aff_idx_2 = sys.argv[5]
    if int(aff_idx_2) < 0 or int(aff_idx_2) > 2:
        print("Invalid input for aff_idx_2" + aff_idx_2)
        sys.exit(1)
    else:
        aff_idx_2 = [int(aff_idx_2), 1 / 3]
    _run_vanilla_main(debug, abrl, gpu, aff_idx_1, aff_idx_2)
