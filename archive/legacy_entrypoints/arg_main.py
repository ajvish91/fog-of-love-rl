...
class Run:
    def __init__(self, check_affinity_condition, gpu, aff_idx_1, aff_idx_2, lambda_, is_abrl=False, debug=False):
        ...
        if is_abrl:
            ...
            self.affinity_indices = {"player_1": [aff_idx_1], "player_2": [aff_idx_2]}
...
def _run_arg_main(debug, abrl, gpu, aff_idx_1, aff_idx_2, lambda_):
    print(gpu, aff_idx_1, aff_idx_2, abrl, debug)
    run = Run(check_affinity_condition, gpu, aff_idx_1, aff_idx_2, lambda_, abrl, debug)
    run.train()
    run.test()
    run.plot_analysis()
...
if __name__ == "__main__":
    if len(sys.argv) >= 3 and sys.argv[1] in ("--config", "-c"):
        ...
        try:
            ...
            sys.exit(1)
        _run_arg_main(debug, abrl, gpu, aff_idx_1, aff_idx_2, lambda_)
        sys.exit(0)

    print(sys.argv)
    if len(sys.argv) != 7:
        print(
            "Usage: python arg_main.py <debug==0|1> <abrl==0|1> <gpu==0-11> "
            "<aff_idx_1==14-20> <aff_idx_2==14-20> <lambda_=float_value>\n"
            "   or: python arg_main.py --config configs/smoke_arg_localized.yaml"
        )
        sys.exit(1)
    debug = sys.argv[1]
    if debug != "1" and debug != "0":
        print("Invalid input for debug" + debug)
        sys.exit(1)
    else:
        debug = True if debug == "1" else False
    abrl = sys.argv[2]

    if abrl != "1" and abrl != "0":
        print("Invalid input for abrl" + abrl)
        sys.exit(1)
    else:
        abrl = True if abrl == "1" else False
    ...
    aff_idx_1 = sys.argv[4]

    if int(aff_idx_1) < 14 or int(aff_idx_1) > 20:
        print("Invalid input for aff_idx" + aff_idx_1)
        sys.exit(1)
    else:
        aff_idx_1 = int(aff_idx_1)

    aff_idx_2 = sys.argv[5]

    if int(aff_idx_2) < 14 or int(aff_idx_2) > 20:
        print("Invalid input for aff_idx" + aff_idx_2)
        sys.exit(1)
    else:
        aff_idx_2 = int(aff_idx_2)
