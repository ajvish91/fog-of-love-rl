...
class Run:
    def __init__(self, check_affinity_condition, is_abrl=False, debug=False):
        ...
        if is_abrl:
            ...
            self.affinity_indices = {"player_1": [14], "player_2": [14]}
    ...
    def get_affinity_success_rate(self, success_rate):
        ...
        for key in self.affinity_indices:
            for index in self.affinity_indices[key]:
        ...
        for success in success_rate:
            for affinity in check_affinity:
                if affinity in success["satisfied"].keys():
                    ...
                    affinity_success_count += 1
                if affinity in success["unsatisfied"].keys():
                    if self.debug:
                        print("failure", affinity, success["unsatisfied"].keys())
                    affinity_failure_count += 1
        
        if not self.debug and affinity_success + affinity_failure != 0:
            self.wandy.log({"affinity_success_rate": affinity_success/(affinity_success+affinity_failure)})
...
if __name__ == "__main__":
    print(sys.argv)
    if len(sys.argv) != 5:
        print("Usage: python script.py <debug==0|1> <abrl==0|1> <gpu==0-11> <aff_idx==14-20>")
        sys.exit(1)
    ...
    gpu = sys.argv[4]
    aff_idx = sys.argv[5]
    run = Run(check_affinity_condition, abrl, debug)
    run.train()
    run.test()
    run.plot_analysis()
    ...
    if abrl:
        ...
        if affinity_success + affinity_failure != 0:
            print("affinity success rate", affinity_success/(affinity_success+affinity_failure))
